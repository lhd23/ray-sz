...
contains
    ...
    subroutine metric(x,g)
        ...
        type(szshell) :: shell
        real(dp) :: t,r,theta,phi,s3,c3,s4,c4,mc3
        real(dp) :: Pp,Qp,Sp,S,RR,Rp,k,ppr,p2
        real(dp) :: f1,f3,f4,f5,fp,grr,gr3,gr4
        t=x(1); r=x(2); theta=x(3); phi=x(4)
        s3=sin(theta); s4=sin(phi)
        c3=cos(theta); c4=cos(phi)
        mc3=1._dp-c3
        Pp=Pp_sz(r); Qp=Qp_sz(r); Sp=Sp_sz(r)
        S=S_sz(r)
    ...
    subroutine tetrad(x,Ec)
        ...
        type(szshell) :: shell
        real(dp) :: t,r,theta,phi,s3,c3,s4,c4,mc3
        real(dp) :: Pp,Qp,Sp,S,RR,Rp,k,ppr
...
! 2nd derivatives of Phi(t,r) divided by Phi(t,r)
        ...
        Qp_S=Qp_sz(r)/S_sz(r)
        Pp_S=Pp_sz(r)/S_sz(r)
        Spp_S=Spp_sz(r)/S_sz(r)
        Qpp_S=Qpp_sz(r)/S_sz(r)
        Ppp_S=Ppp_sz(r)/S_sz(r)
